...
# predictions1 = normalize(predictions1, axis=1, norm='l1')
...
# predictions2 = normalize(predictions2, axis=1, norm='l1')

#get max value from each row for predictions 1 & 2
maxValue_predictions1 = np.zeros((np.sum(phoneme_id==1) + np.sum(phoneme_id==2), 1))
maxValue_predictions2 = np.zeros((np.sum(phoneme_id==1) + np.sum(phoneme_id==2), 1))
for i in range(len(X)):
    maxValue_predictions1[i] = np.sum(predictions1[i])
    maxValue_predictions2[i] = np.sum(predictions2[i])

#get sum of values from each array
predicted_label = np.zeros((np.sum(phoneme_id==1) + np.sum(phoneme_id==2), 1))
predicted_index=0
for i in range(len(X)):
    if maxValue_predictions1[i] > maxValue_predictions2[i]:
        predicted_label[predicted_index] = 1
    elif maxValue_predictions1[i] < maxValue_predictions2[i]:
...
for i in range(len(predicted_label)):
    if predicted_label[i] == pairs_label[i]:
        correct_samples+=1
    total_samples+=1
