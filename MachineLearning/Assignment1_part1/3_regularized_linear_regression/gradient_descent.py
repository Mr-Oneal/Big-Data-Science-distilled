...
def gradient_descent(X, y, theta, alpha, iterations, do_plot,lambda_value):
    ...
    for it in range(iterations):
        ...
        # Make sure to use lambda, if necessary THIS ONE
    ...
    plot_filename = os.path.join(os.getcwd(), 'figures', 'alpha:{}-lambda:{}-predictions.png').format(alpha,lambda_value)
    plt.savefig(plot_filename)
    print('Gradient descent finished.')
    
    # Plot the cost for all iterations
    plot_cost(cost_vector,alpha,lambda_value)
    min_cost = np.min(cost_vector)
    argmin_cost = np.argmin(cost_vector)
    print('Minimum cost: {:.5f}, on iteration #{} for alpha:{} and lambda: {}'.format(min_cost, argmin_cost+1,alpha,lambda_value))
