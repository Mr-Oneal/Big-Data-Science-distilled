...
def gradient_descent(X, y, theta, alpha, iterations, do_plot):
    ...
    m = X.shape[0] # the number of training samples is the number of rows of array X #97
    cost_vector = np.array([], dtype=np.float32) # empty array to store the cost for every iteration

    # Gradient Descent
    for it in range(iterations): #50
        ...
        for i in range(m):
            ...
            hypothesis = calculate_hypothesis(X,theta,i) # the results are going to be 0 for alpha=1
            ########################################/
            output = y[i]
            #print("output :", output)
            sigma = sigma + (hypothesis - output)
            #print("sigma :" , sigma)
        theta_0 = theta_0 - (alpha/m) * sigma #alpha is the lerning rate
        ...
        for i in range(m):
            ...
            ########################################/
            output = y[i]
            sigma = sigma + (hypothesis - output) * X[i, 1] #sigma is the slope
