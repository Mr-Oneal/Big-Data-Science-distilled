...
y = np.array([0, 1, 1, 0])
y_logical_and = np.array([0, 0, 0, 1])
y_logical_nor = np.array([1, 0, 0, 0])
...
errors, nn, _, _ = train(X, y_logical_nor, n_hidden, iterations, learning_rate)
# Test the neural network on the XOR problem
test_xor(X, y_logical_nor, nn)
