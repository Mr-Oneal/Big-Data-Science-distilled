...
class NeuralNetwork():
    ...
    def backward_pass(self, inputs, targets, learning_rate):
        ...
        J = 0.5 * np.sum( np.power(self.y_out - targets, 2) ) #Cost function calculation
        ...
        output_deltas = np.zeros((self.n_out)) #creating an array of 0's for the many deltas, is 1 becuase of the no. of outputs 
        outputs = self.y_out.copy() #outputs in the output neuron, predicted value
        
        for i in range(self.n_out): #we are triying to update the weights of the output neuron
            #########################################
        ...
        for i in range(len(hidden_deltas)):
            ...
            hidden_deltas[i] = sigmoid_derivative(self.y_hidden[i])*summation #we applied the sigmoid derivative to y hidden to get the unsigmoided results
            ########################################/
            #
            #
        ...
        for i in range(len(self.y_hidden)):
            for j in range(len(output_deltas)):
                ...
                self.w_out[i,j] = self.w_out[i,j] - (learning_rate*output_deltas[j]*self.y_hidden[i]) #self.y_hidden[i] has alrady been trough a sigmoid function
                ########################################/
